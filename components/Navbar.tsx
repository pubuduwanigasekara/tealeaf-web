...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose, isScrolled, onNavigate }) => {
    ...
    >
      ...
      */}
      ...
      <div className="container mx-auto px-6 flex-1 overflow-y-auto overflow-x-hidden flex flex-col relative z-10">
        {/* Navigation Links */}
        <div ref={contentRef} className="flex-1 flex flex-col justify-center space-y-6 min-h-[300px]">
            {navLinks.map((link, index) => (
              <a 
                key={link.name} 
                href={link.href}
                className="mobile-link-item group flex items-baseline gap-4 sm:gap-6 pl-2 cursor-pointer"
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-8">
                  ...
                  <div className="flex gap-6 text-brand-gray/40">
                    <a href="https://www.linkedin.com/in/angelajsweeney?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=ios_app" 
                    className="hover:text-brand-primary transition-colors"
                     target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Linkedin"><Linkedin size={24}/></a>
                    <a href="mailto:hello@tealeafconsult.com" className="hover:text-brand-primary transition-colors" aria-label="Email"><Mail size={24} /></a>
                  </div>
                </div>
            </div>
        </div>
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          {/* Logo */}
          <a href="/" onClick={scrollToTop} className="flex items-center gap-3 transition-colors cursor-pointer">
             <img 
               src="https://res.cloudinary.com/deszn12rt/image/upload/v1768411955/tealeaf/logo_quygd6.png" 
               alt="Tealeaf Logo" 
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a 
                ...
                className={`text-sm font-medium ${isScrolled ? 'text-brand-dark/80' : 'text-brand-dark/90'} hover:text-brand-accent transition-colors uppercase tracking-wider cursor-pointer`}
              >
                {link.name}
              </a>
            ))}
