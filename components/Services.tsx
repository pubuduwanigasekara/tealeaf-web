...
const services: ServiceItem[] = [
  ...
  },
  {
    ...
    highlightLine: 'Optimizing performance through strategic financial management.',
  ...
  },
  {
    ...
    description: 'As your CFO partner, we deliver strategic financial leadership to guide fundraising, acquisition opportunities, and long-term planning. Our executive-level analysis and forecasting provide the clarity you need to make confident decisions that accelerate growth and maximize enterprise value.',
    details: [
...
const ServiceCard = React.forwardRef<HTMLDivElement, { service: ServiceItem, index: number }>(({ service, index }, ref) => {
  return (
    // We use position: sticky in CSS for the mechanics, and GSAP for the visual polish.
    // Modified to be relative on mobile, and sticky only on large screens (lg:).
    // Sticky top is 32 (128px).
    // Added larger bottom margin (lg:mb-64) for spacing, handled by ScrollTrigger buffer.
    <div 
      ref={ref}
      className="service-card relative lg:sticky lg:top-24 bg-white border border-brand-dark/5 shadow-lg rounded-3xl overflow-hidden origin-top will-change-transform"
    >
          ...
          <div className="md:col-span-10 flex flex-col">
             ...
             <div className="mb-8">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-brand-accent font-bold tracking-widest uppercase text-xs">{service.subtitle}</p>
                  {/* Step Index - Mobile */}
                  <span className="lg:hidden text-2xl font-serif text-brand-primary/30 font-bold">{service.id}</span>
                </div>
                <h3 className="text-3xl lg:text-4xl font-serif text-brand-dark">{service.title}</h3>
             </div>
             ...
             {/* Capabilities Grid */}
             <div className="mb-12">
                <h4 className="hidden lg:block text-xs font-bold text-brand-primary uppercase tracking-[0.2em] mb-6">Service Deliverables</h4>
                <div className="grid sm:grid-cols-2 gap-x-12 gap-y-4">
                  {service.details.map((detail, idx) => (
                    <div key={idx} className="flex items-center gap-3 group/item">
                      <CheckCircle2 className="w-5 h-5 text-brand-accent mt-0.5 shrink-0 opacity-80" />
                      <span className="text-brand-gray/90 text-sm md:text-base font-medium leading-snug tracking-wide">{detail}</span>
                    </div>
                  ))}
                </div>
             </div>
...
export const Services: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      // Use matchMedia to only apply stacking effect on desktop (lg breakpoint: 1024px)
      ...
      mm.add("(min-width: 1024px)", () => {
          ...
          // We animate the PREVIOUS card based on the scroll position of the NEXT card.
          cards.forEach((card, index) => {
            ...
            // Animate the current card fading out/scaling down as the next one arrives
            ...
            gsap.to(card, {
              ...
              scrollTrigger: {
                trigger: nextCard,
                start: "top 80%", // Start earlier to give more time
                // Sticky top is 32 (128px).
                // We end at 150px to ensure animation finishes just before lock.
                // Large margins on cards require enough scroll space, handled by container padding.
                end: "top top+=150", 
                scrub: true,
  ...
  return (
    <section id="services" ref={containerRef} className="py-24 bg-brand-cream relative">
       <div className="container mx-auto px-6">
         ...
         <div className="mb-16 md:mb-24 max-w-2xl relative z-10">
            <h2 className="text-sm font-bold tracking-widest text-brand-accent uppercase mb-3">Service Offerings</h2>
         ...
         <div className="relative pb-24 space-y-20 lg:space-y-96"> 
