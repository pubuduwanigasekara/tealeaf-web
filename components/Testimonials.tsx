...
const MobileTestimonialCard: React.FC<{ item: TestimonialItem }> = ({
  item,
}) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-brand-dark/5 flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4 border-t border-brand-dark/5 pt-6">
        <div className="h-18 w-18 rounded-full bg-brand-accent/10 relative overflow-hidden border border-brand-primary/20">
          <Image
            src={item.profilePicLink}
            alt={item.author}
            className="w-full h-full object-cover filter grayscale-25"
            loading="lazy"
            fill
            sizes="100px"
          />
      ...
      <div ref={contentOuterRef} className="h-0 overflow-hidden">
        <div
          ...
          <div className="col-span-3 pl-8">
            <div className="space-y-4 animate-[fadeIn_0.5s_ease-out_0.3s_forwards]">
              {/* <div className="h-12 w-[3px] bg-brand-accent/30"></div> */}
              <div className="relative aspect-square max-w-52 overflow-hidden rounded-sm border border-brand-primary/20 shadow-sm">
                <Image
                  src={item.profilePicLink}
                  alt={item.author}
                  className="w-full h-full object-cover filter grayscale-25"
          ...
          <div className="col-span-8 pr-12">
            <div>
              <span className="text-brand-accent/60 pr-2 text-6xl">
                &ldquo;
              </span>
            </div>
